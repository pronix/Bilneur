...
(function($){
	$.fn.extend({ 
		...
		var options = $.extend(defaults, options);
			...
			$('ul', obj).css({'list-style':'none','margin':'0 0 0 175px','padding':'0','position':'relative'}).width(imgWidth*numImages);
			...
			if(o.displayThumbnails)
			{
				function thumbclick(event)
				{
					...
					if(viewable[0] != target_num[1])
					{
						...
						autopilot = 0;
						setTimeout(function(){$('#play_pause_btn'+randID).css('background-position','0 0')},o.transitionSpeed);
			...
			if(o.showControls)
			{
				...
				html = '<div id="play_pause_btn'+randID+'" style="cursor:pointer;position:absolute;top:20%;left:48%;border:none;width:44px;height:44px;background:url('+o.imagePath+'pause.png) no-repeat 0 0"></div>';
				$(obj).append(html);
				var status = 'play';
				...
				html = '<div id="btn_rt'+randID+'" style="position:absolute;right:2px;top:'+arrowsTop+'px;cursor:pointer;border:none;width:35px;height:65px;background:url('+o.imagePath+'leftright.png) no-repeat 0 0"></div>';
				html += '<div id="btn_lt'+randID+'" style="position:absolute;left:2px;top:'+arrowsTop+'px;cursor:pointer;border:none;width:35px;height:65px;background:url('+o.imagePath+'leftright.png) no-repeat -35px 0"></div>';
			...
			function forcePause()
			{
				...
				{
					o.onPauseClick.call(this);
					$('#play_pause_btn'+randID).fadeTo(250,0,function(){$(this).css({'background-position':'0 0','opacity':'.5'});}).animate({opacity:.5},250);
					autopilot = 0;
					showminmax();
					$('#progress'+randID).stop().fadeOut();
					clearTimeout(clearInt);
