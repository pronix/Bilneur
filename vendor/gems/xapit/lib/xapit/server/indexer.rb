  ...
  module Server
    class Indexer
      ...
      def document
        ...
        document.data = id
        terms.each { |term, weight| next if term.blank?;  document.add_term(term, weight) }
        text_terms.each { |term, weight| next if term.blank?; database.add_spelling(term, weight) } if Xapit.config[:spelling]
